...
public class FluentMongoApiTests {
	...
	/**
	 * The difference between the terminating methods {@link TerminatingFindOperation#first()} and
	 * {@link TerminatingFindOperation#one()} is that {@code fist()} returns the very first entry found or none at all.
	 * Therefore the query execution is limited to exactly {@code 1} element via
	 * {@link com.mongodb.client.FindIterable#limit(int)}; <br />
